...
class InfUFluxPipeline:
    def __init__(
        ...
        if quantize and torch.cuda.is_available():
            # Requires bitsandbytes. This flag is often passed via **kwargs
            # directly into from_pretrained in modern diffusers/transformers.
            # We specify device_map='auto' which accelerate uses to handle
            # placing layers, often necessary for quantization.
            load_kwargs["load_in_8bit"] = True
            load_kwargs["device_map"] = "balanced"  # Important for bitsandbytes integration
            print("Attempting to load models in 8-bit precision.")
        else:
            print("Loading models in default precision (Float16).")

        if not os.path.exists("./models/InfiniteYou"):
            print(
                "No InfiniteYou model found. Downloading from HuggingFace `ByteDance/InfiniteYou` to `./models/InfiniteYou` ...")
            snapshot_download(repo_id='ByteDance/InfiniteYou', local_dir='./models/InfiniteYou',
        ...
        try:
            ...
            print("Failed to load InfuseNet model", e)
        ...
        del ipm_state_dict
        image_proj_model.to(self.device, torch.float16)
    ...
    def __call__(
        ...
        id_embed = id_embed.clone().unsqueeze(0).float().to(self.device)
        id_embed = id_embed.reshape([1, -1, 512])
        id_embed = id_embed.to(dtype=torch.float16)
        with torch.no_grad():
